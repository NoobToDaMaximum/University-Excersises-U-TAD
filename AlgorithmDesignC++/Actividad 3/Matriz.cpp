...
double Matriz::calcularDeterminante(){
  ...
  if(n_columnas == n_filas){
    a = this->matriz[0][0] * this->matriz[1][1] + this->matriz[0][1] * this->matriz[1][0];
  }else{
    Matriz *m1 = new Matriz(n_columnas-1, n_filas-1);
    for(int i = 0; i < n_columnas; i++){
      for(int row = 1; row < n_filas; row++){
        for(int column = 0; column < n_columnas; column++){
          if(column != i){
            m1->matriz[column-1][row] = matriz[row][column];
          }
        }
        if(i%2 == 0){
          a += this->matriz[0][i] * m1->calcularDeterminante();
        }
        if(i%2 != 0){
          a -= this->matriz[0][i] * m1->calcularDeterminante();
        }
